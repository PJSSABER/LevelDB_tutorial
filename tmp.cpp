 ...
 */
void eventSet(struct myevent_s *ev, int fd, int events, Callback_t call_back,
              void *arg, int index) {
  ev->fd = fd;
...
void senddata(int fd, int events, void *arg) {
  ...
  if (nbytes > 0) {
    /// 设置 抽象事件 注册回调函数 并绑定到 epoll 事件多路分发器 （这里是
    /// read事件） 回调函数 recvdata 处理 read事件
...
void recvdata(int fd, int events, void *arg) {
  ...
  if (nbytes > 0) {
    ...
    ev->len = len + 1;
    ev->buf[len] = 0;
    /// 设置 抽象事件 注册回调函数 并绑定到 epoll 事件多路分发器 （这里是 send
    /// 事件） 回调函数 senddata 处理 send事件
...
void initlistensocket(int epfd, unsigned short port) {
  ...
  printf("fd= %d\n", lfd);
  /// 设置 抽象事件 注册回调函数 并绑定到 epoll 事件多路分发器 （这里是 accept
  /// 事件） accept 事件触发 注册的回调函数 acceptConn 处理 这里 使用 MAX_EVENTS
  /// 下标的抽象事件 作为 accept 事件
